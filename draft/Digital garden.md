Тема ведения заметок в последнее время стала популярной.
...
В статье я расскажу о том, что такое сад, из чего его растить, зачем нужно делиться и как это делать.
...
## Мой случай, мои заметки, как я веду заметки

Всем привет, хочу поделиться своим опытом и рассказать о концепции цифрового сада, с которой недавно познакомился.  

Узнал от https://github.com/rwxrob на твиче.

Последние три года веду заметки в стиле Zettelkasten (на сколько я его понял и принял к использованию). Заметки в виде md файлов храню в git репозитории. На компьютере работаю с ними в VSCode, на телефоне использую стандартное приложение "Заметки". Почему не Obsidian? У меня около 1000 заметок, и каждый раз, когда открываю приложение на iPhone, Obsidian может до минуты синхронизироваться, возможно из-за конфликта с git метаданными. Мне нужно быстрое приложение заметок, чтобы записывать мысли на ходу. Использование отдельного приложения на телефоне имеет плюс — при переносе в md файлы я лишний раз пересматриваю заметку и лучше запоминаю написанное. Перенос заметки из одной условной "папки" в другую это часто повторяющийся паттерн работы с материалом, который я видел много где, начиная от Джедайских Техник Дорофеева, до Библии комедии (ты уверен?)

Моя коллекция заметок = In this collection, you’ll find code snippets, review materials, and sometimes just headers as reminders to revisit topics. It mainly covers software engineering, with a few snippets in Java and Go. You’ll also find philosophy from the Hellenes (a fancy way to say “ancient Greeks”), silly jokes, and other interesting things I’ve encountered. Most of it is in Russian, with some in English.
