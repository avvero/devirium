...
- Testing Trophy подход, почему?
- Pile technic
- Testing Driven Development, почему?

Все эти техники очень связаны и взаимно дополняют друг друга.

### Testing Trophy

Это подход к покрытию кода тестами таким образом, что в первую очередь акцент делается на интеграционные тесты, проверяющие контракт сервиса в целом. Unit тесты используются для конкретных отдельных методов, которые сложно или дорого тестировать через интеграционные тесты. Тесты с подобным подходом я описывал в своих статьях: https://habr.com/ru/articles/781812/, https://habr.com/ru/articles/804673/, https://habr.com/ru/articles/797049. 

### Pile technic
### Testing Driven Development

Идеально сочетается с "Pile technic" - на первом этапе мы концентрируемся на проверке и реализации ожидаемого поведения, сваливая весь код в кучу, а когда все тесты написаны и код контракт выполняет, наступает этап рефакторинг, в рамках которого мы при наличии всего нужного кода во всем его многообразии, мы можем построить необходимый и достаточный дизайн. А не пытаться загодя, до того как код написал, какие-то наметки дизайна делать. Потому что мы скорее всего ошибемся, но прочерченные линии дизайна в виде классов и методов уже не дадут нам так легко охватить код и описать его заново.
