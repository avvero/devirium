...
Время выполнения теста - это всего лишь синтетическая метрика, которой недостаточно для принятия обоснованного  решения о приоритете различных видов тестирования. Наша главная цель - не просто «прогнать тесты и сделать это быстро», а гарантировать соответствие поведения требованиям (как функциональным, так и нефункциональным).

Не затрагиваются прочие кретерии оценки тестов. Речь о том, чтобы указать на тот факт, что оценивать тесты только по времени выполнения не достаточно.
